...
fun AddPost(
      ...
      CustomButton(
        ...
        onClick = {
          ...
          if (!nameError && !descError && !categoryError) {
            isLoading = true
            scope.launch {
              val addPostSuccessful = vm.createNewItem()
              isLoading = false
  ...
  if (showSuccessDialog) {
    CustomDialog(
      ...
      body = stringResource(id = R.string.add_post_success_desc),
  ...
  if (showFailedDialog) {
    CustomDialog(
      title = stringResource(id = R.string.add_post_error_header),
      body = stringResource(id = R.string.add_post_error_desc),
